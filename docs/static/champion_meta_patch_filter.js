(function () {
  ...
  function patchOptions(data) {
    ...
    if (options.length) return [{ value: 'all', label: '-- ALL S16 --' }, ...options];
    const patch = String(data?.patch || '').trim();
    return isSeason16Patch(patch) ? [{ value: patch, label: patch }] : [{ value: 'all', label: '-- ALL S16 --' }];
  }
  ...
  function summaryScope(data) {
    const summaries = data?.patch_summaries && typeof data.patch_summaries === 'object' ? data.patch_summaries : {};
    return summaries[selectedPatch] || (selectedPatch === 'all' ? season16Summary(data) || summaries.all : null) || data || {};
  }

  function patchText(scope) {
    if (scope?.patch === 's16') return 'S16 patches';
