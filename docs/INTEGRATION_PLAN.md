...
- Expose the active prediction feed and, when override URLs are configured, a separate remote analyzer feed/schema probe so local bootstrap fallback does not hide a broken external analyzer host. Implemented.
...
- `/api/diagnostics` defaults to Cloudflare-hosted artifact URLs on the same Pages origin. When remote override URLs are configured, it reports probe fields such as `remote_prediction_feed_available`, `remote_prediction_feed_last_fetch_status`, and `remote_prediction_schema_ok`; remote failures are surfaced through `artifact_warnings` without breaking local fallback.
