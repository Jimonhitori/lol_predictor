...
.lineupPlayers { display: grid; grid-template-columns: repeat(5, minmax(64px, 1fr)); gap: 0; border: 1px solid #8b5fbf; border-radius: 6px; overflow: hidden; background: #252d37; }
...
.playerFlag { font-size: 11px; margin-left: 2px; }
.starterTag { background: #315a4a; color: #86efac; border-radius: 5px; padding: 3px 8px; font-size: 11px; font-weight: 800; }
...
function lineupPlayers(players) {
  ...
  return players.map(player => `
    <div class="lineupPlayer" title="${escapeHtml(player.games)} games · ${(player.winrate * 100).toFixed(1)}% WR · KDA ${Number(player.kda).toFixed(2)} · ${escapeHtml(player.top_champions.join(', ') || '-')}">
      ...
      <span class="playerName">${escapeHtml(player.player)} <span class="playerFlag">🇰🇷</span></span>
      <span class="starterTag">スタメン</span>
    </div>
  `).join('');
}
